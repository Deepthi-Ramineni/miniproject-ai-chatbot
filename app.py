...
for index_name in [PINECONE_INDEX_FLAT, PINECONE_INDEX_DAA, PINECONE_INDEX_APTITUDE]:
...
vectorstore_flat = PineconeVectorStore.from_existing_index(index_name=PINECONE_INDEX_FLAT, embedding=embeddings)
retriever_flat = vectorstore_flat.as_retriever(search_kwargs={"k": 10})

vectorstore_daa = PineconeVectorStore.from_existing_index(index_name=PINECONE_INDEX_DAA, embedding=embeddings)
retriever_daa = vectorstore_daa.as_retriever(search_kwargs={"k": 10})

vectorstore_apt = PineconeVectorStore.from_existing_index(index_name=PINECONE_INDEX_APTITUDE, embedding=embeddings)
...
def flat_ask_question():
    ...
    search_results = pc.Index(PINECONE_INDEX_FLAT).query(
...
def daa_ask_question():
    ...
    conversation_history.append({"role": "user", "content": question})
    question_embedding = embeddings.embed_query(question)
    search_results = pc.Index(PINECONE_INDEX_DAA).query(
        vector=question_embedding,
        top_k=5,
        include_metadata=True
    )
...
def aptitude_ask_question():
    ...
    question_embedding = embeddings.embed_query(question)
    search_results = pc.Index(PINECONE_INDEX_APTITUDE).query(
        vector=question_embedding,
        top_k=5,
        include_metadata=True
    )
